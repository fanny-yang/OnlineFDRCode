...
class GAI2_MW_AD_proc_batch: 
    ...
    def __init__(self, alpha0, numhyp, startfac, pr_w, pen_w, mempar, abstain_vec, abs_eps, wealth_eps):
        pen_min = min(pen_w)
        self.alpha0 = alpha0
        ...
        self.pr_w = pr_w
        self.pen_w = pen_w
        # Either the abstinence vector is known beforehand, then here (or adaptive)
        self.abstain = abstain_vec
        self.abs_eps = abs_eps
        self.wealth_eps = wealth_eps
    ...
    def run_fdr(self, pvec):
        ...
        for k in range(0, numhyp):
            ...
            if self.wealth_vec[k] < self.wealth_eps*self.w0:
 
                if (rej_sum[k] < self.abs_eps) and self.abstain[k] == 0:
                
                    # print("Resetting at k = %d with wealth = %5.f and rejection sum %.3f " % (k, self.wealth_vec[k], rej_sum[k]))
                    # reset everything
                    wealth = self.w0
                    rej_sum[k + 1] = 0
                    next_alpha = self.gamma_vec[0]*self.w0

                else:
                    # # Abstain, i.e. no rejection, no test
                    # print("Abstaining at k = %d with wealth = %.5f and rejection sum %.3f" % (k, self.wealth_vec[k], rej_sum[k]))
                    # Update wealth
                    wealth = self.mempar*self.wealth_vec[k] + (1-self.mempar)*flag*self.w0 
                    # Forget past rejections 
                    rej_sum[k + 1] = self.mempar*rej_sum[k]
                    # the original alpha gets carried over to the next one
                    next_alpha = self.alpha[k + 1]
                
            elif self.wealth_vec[k] > 0:
                ...
                rej_sum[k + 1] = rej_sum[k] + rej[k + 1]
                if (rej[k + 1] == 1):
                    if (first == 0):
                        first = 1
                        #b_k = b_k - np.true_divide(self.w0, pen_w[k])
                ...
                if len(last_rej) > 0:
                    ...
                    # #sum_gam = sum(np.multiply(self.mempar**t_taoj, self.gamma_vec[t_taoj])) - first_gam 
